 ...
 * that entry.
 */
const RouterRenderer = ({ loader }: RouterRendererProps): Node => {
  // Access the router
  ...
  const firstItem: any = reversedItems[0];
  ...
  return (
    <ErrorBoundary>
      <Suspense fallback={loader ?? 'Loading fallback...'}>
        {/* Indicate to the user that a transition is pending, even while showing the previous UI */}
        {isPending ? (
          <div className="RouterRenderer__loading">{loader ?? 'Loading pending...'}</div>
        ) : null}
        {routeComponent}
      </Suspense>
    </ErrorBoundary>
